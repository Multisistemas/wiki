    ...
    function handle_revisions(Doku_Event &$event, $param) {
        ...
        foreach ($event->data->_content as $key => $ref) {
            ...
            if ($member && $ref['_elem'] == 'tag' &&
                ...
                } elseif ($this->hlp->isRevisionReviewed($revision)) {
                    $event->data->_content[$member]['class'] = 'li reviewed_revision';
                } else {
                    $event->data->_content[$member]['class'] = 'li unreviewed_revision';
                } 
